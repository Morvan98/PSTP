...
def seq_list_2_embedding(seq_list_fixlen,chunk_size=chunk_size_):
    ...
    for chunk_i in range(0,len(data_),chunk_size):
        ...
        batch_tokens = batch_tokens.to(dev1ce)
        with torch.no_grad():
...
# uniprot_seq_all_file = 'data_collection/uniprot_human.fasta'
# uniprot_entry2seq = {}
...
#         uniprot_entry2seq[uniprot_entry] = str(record.seq)
#     except:
#         pass

# seq_test = []
# for u_entry,seq_ in zip(uniprot_entry2seq.keys(),uniprot_entry2seq.values()):
#     seq_test.append(seq_[:window_size])

# seq_representations = seq_list_2_embedding(seq_test[:1000])
# # print(np.array(seq_representations))
# # print(np.array(seq_representations).shape)
# exit()
# print(seq_representations.shape)
####
...
def get_seq_embedding(seq_list,winsize=256):
    window_size = winsize
    ...
    short_seqs_embeddings = seq_list_2_embedding(short_seqs_lst)
    idx2embeddings = dict(zip(short_seqs_idx_lst,short_seqs_embeddings))
    # print(f'embedding seq len > {window_size}')
    long_seqs_embeddings = []
    for item in long_seqs:
        long_seq = item[1]
        sub_seqs = generate_sliding_sequences(long_seq,window_size=window_size)
        embeddings = np.average(seq_list_2_embedding(sub_seqs,len(sub_seqs)),0)
...
################ test embedding IDRs with different compaction ###############
if __name__ == '__main__':
    
    test_seq_lst = ['RNKQVKDVFKHTVKKIELFSMK']
    
    # test_seq = ['MRNHTTVANFILLGLTDDPQLQ','MRNHTTVAN']
    # test_seq = ['MRNHTTVAN']
    results = np.array(get_seq_embedding(test_seq_lst))
    test_seq = test_seq_lst[0]
    print(len(test_seq))
    test_result = _get_seq_embedding_matrix_long_seq_input(test_seq,2,10)
    ...
    results = get_seq_poswide_embedding_matrix(seqs)
    for l in results:
        print('hello')
        for t in l:
            print(np.min(t))
